...
class AnalyticsService {
  ...
  initialize(measurementId: string): void {
    ...
    try {
      ...
      window["gtag"] = function() { (window as any).dataLayer.push(Array.from(arguments)) }
      ...
      switch (event.event_type) {
        ...
        case 'premium_request_started':
          window["gtag"]('event', 'add_to_cart', {
            ...
            items: [{
              item_id: `premium_${event.duration_months ?? 1}m`,
              item_name: `Premium ${event.duration_months ?? 1} Month${(event.duration_months ?? 1) > 1 ? 's' : ''}`,
