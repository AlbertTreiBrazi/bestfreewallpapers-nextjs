...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [
    ...
    supabase.from('live_wallpaper_categories').select('slug, updated_at').eq('is_active', true),
    // Tags (array columns — fetch and flatten in JS)
    supabase.from('wallpapers').select('tags').eq('is_active', true).not('tags', 'is', null),
    supabase.from('ringtones').select('tags').eq('is_active', true).eq('is_published', true).not('tags', 'is', null),
    supabase.from('live_wallpapers').select('tags').eq('is_active', true).eq('is_published', true).not('tags', 'is', null),
  ])
  ...
  const liveCatUrls = (liveCatsRes.data || []).map((c) => ({
    url: `${SITE_URL}/live-wallpapers/category/${c.slug}`,
    lastModified: new Date(c.updated_at),
    changeFrequency: 'weekly' as const,
    priority: 0.6,
  }))

  // ── Tag pages — deduplicate across all content types ──────────────────────

  const rawTags = [
    ...(tagsWallRes.data || []).flatMap((r) => r.tags ?? []),
    ...(tagsRingRes.data || []).flatMap((r) => r.tags ?? []),
    ...(tagsLiveRes.data || []).flatMap((r) => r.tags ?? []),
  ]
  const uniqueTags = [...new Set(rawTags.map((t) => String(t).trim().toLowerCase()))].filter(Boolean)
  ...
  const staticUrls: MetadataRoute.Sitemap = [
    ...
    { url: `${SITE_URL}/live-wallpapers/how-to-set`,                         lastModified: new Date(), changeFrequency: 'monthly', priority: 0.6 },
