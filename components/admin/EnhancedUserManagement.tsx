...
export function EnhancedUserManagement() {
  ...
  const loadUsers = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (filters.planType !== 'all') {
        users = users.filter((user: any) => user.profile?.plan_type === filters.planType)
      }
      
      if (filters.adminStatus !== 'all') {
        const isAdmin = filters.adminStatus === 'admin'
        users = users.filter((user: any) => !!user.profile?.is_admin === isAdmin)
      }
      
      if (filters.hasVideoDownload !== 'all') {
        const hasVideo = filters.hasVideoDownload === 'true'
        users = users.filter((user: any) => !!user.profile?.has_video_download === hasVideo)
      }
  ...
  const handleSearchChange = (value: string) => {
    ...
    setPagination((prev: any) => ({ ...prev, page: 1 })) // Reset to first page
  }
  ...
  const selectAllUsers = () => {
    if (selectedUsers.size === users.length) {
      ...
      setSelectedUsers(new Set(users.map((user: any) => user.user_id)))
    }
  }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditingUser((prev: any) => prev ? { ...prev, full_name: e.target.value } : null)}
                  className="w-full p-2 border border-theme-light rounded-lg focus:ring-2 focus:ring-blue-500 bg-theme-surface text-theme-primary"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingUser((prev: any) => prev ? { ...prev, plan_type: e.target.value } : null)}
                  className="w-full p-2 border border-theme-light rounded-lg focus:ring-2 focus:ring-blue-500 bg-theme-surface text-theme-primary"
                >
                  <option value="free">Free</option>
                  <option value="premium">Premium</option>
              ...
              <div>
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setEditingUser((prev: any) => prev ? { ...prev, is_admin: e.target.checked } : null)}
                    className="rounded border-theme-light"
                  />
              ...
              <div>
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setEditingUser((prev: any) => prev ? { ...prev, has_video_download: e.target.checked } : null)}
                    className="rounded border-theme-light"
                  />
