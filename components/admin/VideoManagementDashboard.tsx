...
export function VideoManagementDashboard() {
  ...
  const loadVideoData = async () => {
    ...
    try {
      ...
      // Apply type filter
      if (filterType === 'local') {
        filteredVideos = filteredVideos.filter((video: any) => video.local_video_path)
      } else if (filterType === 'external') {
        filteredVideos = filteredVideos.filter((video: any) => video.live_video_url && !video.local_video_path)
      }
      ...
      const localVideos = filteredVideos.filter((v: any) => v.local_video_path).length
      const externalVideos = filteredVideos.filter((v: any) => v.live_video_url && !v.local_video_path).length
