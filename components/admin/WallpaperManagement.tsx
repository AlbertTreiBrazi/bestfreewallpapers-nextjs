...
export function WallpaperManagement() {
  ...
  const loadCollections = async () => {
    ...
    try {
      ...
      const mappedCollections = (data.data || []).map((collection: any) => ({
        ...collection,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        tags: formData.tags
          ? formData.tags.split(',').map((tag: any) => tag.trim()).filter(Boolean)
          : [],
  ...
  const handleCollectionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/collections-management`,
        {
                  ...
                  <div className="p-4">
                    ...
                    <div className="border-t border-theme-light pt-3">
                      <CollectionCoverUpload
                        ...
                        currentCoverUrl={(collection.cover_image_url || collection.thumbnail_url) ?? undefined}
                        onUploadSuccess={(url) => {
                  ...
                  >
                    <span className={formData.category_id ? 'text-theme-primary' : 'text-theme-tertiary'}>
                      {formData.category_id 
                        ? categories.find((cat: any) => cat.id.toString() === formData.category_id)?.name || 'Unknown Category'
              ...
              <div className="space-y-3">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setCollectionFormData((prev: any) => ({ ...prev, is_featured: e.target.checked }))}
                    className="rounded text-gray-600 focus:ring-gray-500"
                  />
                ...
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setCollectionFormData((prev: any) => ({ ...prev, is_published: e.target.checked }))}
                    className="rounded text-gray-600 focus:ring-gray-500"
                  />
                  <span className="ml-2 text-sm font-medium text-theme-secondary">Published</span>
                </label>
