...
export function CollectionCoverUpload({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/collections-cover-upload`,
        {
          method: 'POST',
          headers: {
  ...
  const handleAutoThumbnail = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/collections-auto-thumbnail`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.data.session.access_token}`,
