...
export function EnhancedAdminPanel() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      if (activeTab === 'dashboard') {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/admin-metrics`, {
          method: 'GET',
          headers: {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/admin-dashboard?${params}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
            'Content-Type': 'application/json'
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/admin-dashboard?action=admins`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
            'Content-Type': 'application/json'
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/admin-rate-limits`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
            'Content-Type': 'application/json'
  ...
  const handleInviteAdmin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/invite-admin`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center shadow-lg shadow-green-500/20">
                <Shield className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="text-lg font-bold text-white leading-tight">Admin Panel</h1>
                <p className="text-xs text-gray-500">bestfreewallpapers.com</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="hidden sm:flex items-center gap-1.5 text-xs text-green-400 bg-green-400/10 border border-green-400/20 px-3 py-1.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
                {profile?.admin_role === 'super_admin' ? 'Super Admin' : 'Admin'}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setInviteFormData((prev: any) => ({ ...prev, email: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setInviteFormData((prev: any) => ({ ...prev, full_name: e.target.value }))}
                  className="w-full p-2 border border-theme-light rounded-lg focus:ring-2 focus:ring-blue-500 bg-theme-surface text-theme-primary"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setInviteFormData((prev: any) => ({ ...prev, role: e.target.value }))}
                  className="w-full p-2 border border-theme-light rounded-lg focus:ring-2 focus:ring-blue-500 bg-theme-surface text-theme-primary"
                >
                  <option value="moderator">Moderator</option>
                  <option value="admin">Admin</option>
                ...
                <div className="space-y-2">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setInviteFormData((prev: any) => ({
                        ...prev,
