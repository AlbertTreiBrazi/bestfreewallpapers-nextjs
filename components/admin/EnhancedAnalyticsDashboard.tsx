...
function EnhancedAnalyticsDashboard() {
  ...
  const loadAnalytics = async (forceRefresh = false) => {
    setAnalytics((prev: any) => ({ ...prev, loading: true, error: null }))
    ...
    try {
      ...
      const [statsResponse, trendsResponse, topDownloadsResponse] = await Promise.all([
        ...
        fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/admin-metrics${forceRefresh ? '?force=true' : ''}`, {
          method: 'GET',
          headers: {
      ...
      setAnalytics((prev: any) => ({ ...prev, loading: false, error: error.message }))
  ...
  const enhanceStatsWithDownloadBreakdown = async (stats: AnalyticsStats): Promise<AnalyticsStats> => {
    try {
      ...
      const guestDownloads = downloads?.filter((d: any) => d.user_id === null).length || 0
      const premiumDownloads = downloads?.filter((d: any) => d.user_type === 'premium').length || 0
      const freeUserDownloads = downloads?.filter((d: any) => d.user_id !== null && d.user_type !== 'premium').length || 0
  ...
  const loadTrendData = async (): Promise<TrendData[]> => {
    try {
      ...
      const trendData: TrendData[] = last30Days.map((date: any) => {
        ...
        const dayPremiumUsers = users?.filter((u: any) => 
          u.created_at.startsWith(date) && 
          u.plan_type === 'premium' &&
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-7 gap-2">
                {analytics.trends.slice(-7).map((trend, index) => {
                  const maxDownloads = Math.max(...analytics.trends.map((t: any) => t.downloads))
