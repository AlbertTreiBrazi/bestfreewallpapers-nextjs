...
export function SecurityDashboard() {
  ...
  const loadSecurityMetrics = async () => {
    try {
      ...
      const failedAttempts = securityLogs?.filter((log: any) => !log.success).length || 0
      const uniqueIPs = new Set(securityLogs?.map((log: any) => log.ip_address)).size
  ...
  const getSystemHealth = async () => {
    ...
    try {
      ...
      setSecurityMetrics((prev: any) => prev ? {
        ...prev,
        avgResponseTime: 0,
        errorRate: 0
      } : null)
